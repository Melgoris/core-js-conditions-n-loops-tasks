...
function isContainNumber(num, digit) {
  ...
  for (let i = 0; i < numbers.length; i += 1) {
    if (+numbers[i] === digit) return true;
  }
  return false;
}
...
function sortByAsc(arr) {
  const arrlink = arr;
  for (let i = 0; i < arrlink.length - 1; i += 1) {
    let minIndex = i;
    for (let j = i + 1; j < arrlink.length; j += 1) {
      if (arrlink[j] < arr[minIndex]) minIndex = j;
    }
    const temp = arrlink[i];
    arrlink[i] = arrlink[minIndex];
    arrlink[minIndex] = temp;
  }
  return arr;
}
...
function shuffleChar(str, iterations) {
  ...
  for (let i = 0; i < iterations; i += 1) {
    let even = '';
    let odd = '';
    for (let j = 0; j < strLink.length; j += 1) {
      if (j % 2 === 0) even += strLink[j];
      else odd += strLink[j];
    }
    strLink = even + odd;
  }
